 ...
 * Streaming TTS — returns audio as it's generated for lower latency.
 */
voiceRouter.post("/tts-stream", async (req, res) => {
  const { text, voiceId } = req.body || {};
  ...
  try {
    ...
    const { stream, contentType } = await engine.synthesizeStream(text, voiceId);
    res.set("Content-Type", contentType);
...
/**
 * POST /api/voice/fast-chat
 * Ultra-low-latency voice chat — direct Anthropic API (no Agent SDK).
 * Returns { text, audio } where audio is streamed TTS.
 */
voiceRouter.post("/fast-chat", async (req, res) => {
  ...
  try {
    ...
    if (!accessToken) {
      const configDir = process.env.CLAUDE_CONFIG_DIR || path.default.join(process.env.HOME || "/home/node", ".claude");
      try {
        const creds = JSON.parse(fs.default.readFileSync(path.default.join(configDir, ".credentials.json"), "utf8"));
        accessToken = creds.claudeAiOauth?.accessToken;
      } catch { /* no creds */ }
    }
    if (!accessToken) return res.status(401).json({ error: "Not authenticated" });

    // Get conversation history for context (last 10 messages only — speed)
    const db = req.app.get("db");
    const { ConversationStore } = await import("../db/conversations.js");
    const convStore = new ConversationStore(db);
    const history = conversationId ? convStore.getRecentMessages(conversationId, 10) : [];

    // Build messages
    const messages = history.map(m => ({ role: m.role, content: m.content }));
    messages.push({ role: "user", content: message });

    // Direct Anthropic API call — no Agent SDK, no CLI spawn
    const apiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model: "claude-haiku-4-5", // Fastest model for voice
        max_tokens: 300, // Short responses for voice
        system: "You are Tarsee, a conversational AI assistant. Respond in 1-2 short sentences. Be natural, concise, and conversational. No markdown, no lists, no code blocks — just spoken language.",
        messages,
      }),
    ...
    if (!apiRes.ok) {
      ...
      return res.status(apiRes.status).json({ error: err.slice(0, 200) });
    }
    ...
    if (conversationId) {
      convStore.addMessage(conversationId, { role: "user", content: message });
      convStore.addMessage(conversationId, { role: "assistant", content: text });
    }

    // Stream TTS audio
    const engine = getTTSEngine();
    if (engine.synthesizeStream) {
      const { stream, contentType } = await engine.synthesizeStream(text);
      // Return text + audio in multipart-ish response
      // For simplicity, return text first then stream audio
      res.set("Content-Type", "audio/mpeg");
      res.set("X-Voice-Text", Buffer.from(text).toString("base64"));
      for await (const chunk of stream) {
        res.write(chunk);
      }
      res.end();
    } else {
      const { audio, contentType } = await engine.synthesize(text);
      res.set("Content-Type", contentType);
      res.set("X-Voice-Text", Buffer.from(text).toString("base64"));
      res.send(audio);
    }
